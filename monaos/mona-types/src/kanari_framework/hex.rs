...
pub fn encode(bytes: &[u8]) -> Vector<u8> {
    let mut result = Vector::empty();
    for &byte in bytes {
        let hex_bytes = HEX[byte as usize];
        for &b in hex_bytes {
            result.push_back(b);
        }
    }
...
pub fn decode(hex: &[u8]) -> Result<Vector<u8>, HexError> {
    if hex.len() % 2 != 0 {
    ...
    let mut i = 0;
    while i < hex.len() {
        let high = decode_byte(hex[i])?;
        let low = decode_byte(hex[i + 1])?;
...
fn decode_byte(hex: u8) -> Result<u8, HexError> {
    match hex {
        b'0'..=b'9' => Ok(hex - b'0'),
