...
fn create_mock_transaction(sender: &Address, receiver: &Address, amount: u64) -> Transaction {
...
mod tests {
    ...
    fn test_single_client_tps() {
        ...
        println!(
            "Single client TPS (real transactions): {:.2} TPS",
            actual_tps
        );
        println!(
            "Duration: {:?} for {} transactions",
            duration,
            transactions.len()
        );
        ...
        if actual_tps >= expected_min_tps {
            ...
            println!(
                "⚠️ NOTE: Current TPS ({:.2}) is below target (100K). This is a performance benchmark, not a test failure.",
                actual_tps
            );
        }
    }
    ...
    fn test_multi_client_tps() {
        ...
        println!(
            "Multi-client TPS (real transactions): {:.2} ({}/{} successful)",
    ...
    fn benchmark_max_tps() {
        ...
        println!(
            ...
            gen_time,
            transaction_count as f64 / gen_time.as_secs_f64()
        );
        ...
        println!(
            "🚀 MAXIMUM TPS BENCHMARK: {:.2} transactions per second",
            tps
        );
        println!(
            "Submission time: {:?} for {} transactions",
            submit_time,
        ...
        if tps >= 100000.0 {
            ...
            println!(
                "⚠️ Current performance: {:.2} TPS - Optimization opportunities exist",
                tps
            );
        }
    }
    ...
    fn benchmark_mock_transactions_tps() {
        ...
        let gen_time = gen_start.elapsed();
        println!(
            "Mock transaction generation: {:?} ({:.2} tx/s)",
            gen_time,
        ...
        println!(
            "🚀 MOCK TRANSACTION TPS: {:.2} transactions per second",
            tps
        );
        println!(
            "Submission time: {:?} for {} transactions",
