...
pub fn stake_tokens(
    ...
    // Update staking pool
    {
        ...
        let mut balances = BALANCES
            .lock()
            .map_err(|_| BlockchainError::Transaction("Failed to lock balances".to_string()))?;
    ...
    // Update staking data with security hash
    {
        ...
        let mut staking_pool = STAKING_POOL
            .lock()
            .map_err(|_| BlockchainError::Transaction("Failed to lock staking pool".to_string()))?;
...
pub fn unstake_tokens(address: &Address) -> Result<(u64, u64), BlockchainError> {
    ...
    if current_time < unlock_time {
        ...
        early_unlock_penalty = staked_amount.checked_div(10).ok_or_else(|| {
            BlockchainError::Transaction("Division error in penalty calculation".to_string())
        })?;
        withdrawal_amount = withdrawal_amount
            .checked_sub(early_unlock_penalty)
            .ok_or_else(|| {
                BlockchainError::Transaction("Underflow in withdrawal calculation".to_string())
            })?;
        ...
        if early_unlock_penalty > 0 {
            let pool_address = match normalize_address(POOL_ADDRESS) {
                ...
                Err(_) => {
                    error!(
                        "Invalid pool address, penalty lost: {} KA",
                        early_unlock_penalty
                    );
            ...
            if !pool_address.is_empty() {
                ...
                *pool_balance =
                    pool_balance
                        .checked_add(early_unlock_penalty)
                        .ok_or_else(|| {
                            BlockchainError::Transaction(
                                "Overflow in pool penalty addition".to_string(),
                            )
                        })?;
            }
        }
    ...
    // Return tokens to user's balance (minus penalty if applicable)
    {
        ...
        let total_return = withdrawal_amount
            .checked_add(accumulated_rewards)
            .ok_or_else(|| {
                BlockchainError::Transaction(
                    "Integer overflow in withdrawal calculation".to_string(),
                )
            })?;
        let current_balance = balances.entry(address_str.clone()).or_insert(0);
        *current_balance = current_balance.checked_add(total_return).ok_or_else(|| {
            BlockchainError::Transaction("Integer overflow in balance update".to_string())
        })?;
    }
...
pub fn process_rewards(block_height: u32) -> Result<u64, BlockchainError> {
...

    {
        ...
        for (address, node) in staking_nodes.iter_mut() {
            ...
            let reward =
                (node.staked_amount as f64 * daily_reward_rate * days_passed).round() as u64;
    ...
    // Transfer rewards from pool to validators
    {
        ...
        if let Some(pool_balance) = balances.get_mut(&pool_addr_str) {
            ...
            *pool_balance -= total_rewards;
            ...
            return Err(BlockchainError::Transaction(format!(
                "Pool address not found in balances"
            )));
        }
...
fn save_staking_state() -> Result<(), BlockchainError> {
    ...
    let nodes = match STAKING_NODES.read() {
        ...
        Err(_) => {
            return Err(BlockchainError::Storage(
                "Failed to read staking nodes".to_string(),
            ));
        }
    };
    ...
    let pool = match STAKING_POOL.lock() {
        ...
        Err(_) => {
            return Err(BlockchainError::Storage(
                "Failed to read staking pool".to_string(),
            ));
        }
    };
    ...
    let validators = match ACTIVE_VALIDATORS.read() {
        ...
        Err(_) => {
            return Err(BlockchainError::Storage(
                "Failed to read active validators".to_string(),
            ));
        }
    };
    ...
    let hashes = match NODE_HASH_CACHE.read() {
        ...
        Err(_) => {
            return Err(BlockchainError::Storage(
                "Failed to read node hash cache".to_string(),
            ));
        }
    };
    ...
    let nodes_data = match bincode::serialize(&nodes) {
        ...
        Err(e) => {
            return Err(BlockchainError::Storage(format!(
                "Failed to serialize staking nodes: {}",
                e
            )));
        }
    };
    ...
    let pool_data = match bincode::serialize(&pool) {
        ...
        Err(e) => {
            return Err(BlockchainError::Storage(format!(
                "Failed to serialize staking pool: {}",
                e
            )));
        }
    };
    ...
    let validators_data = match bincode::serialize(&validators) {
        ...
        Err(e) => {
            return Err(BlockchainError::Storage(format!(
                "Failed to serialize active validators: {}",
                e
            )));
        }
    };
    ...
    let hashes_data = match bincode::serialize(&hashes) {
        ...
        Err(e) => {
            return Err(BlockchainError::Storage(format!(
                "Failed to serialize node hash cache: {}",
                e
            )));
        }
    };
    ...
    if let Err(e) = storage.save_data(b"active_validators", &validators_data) {
        return Err(BlockchainError::Storage(format!(
            "Failed to save active validators: {}",
            e
        )));
    }
    ...
    if let Err(e) = storage.save_data(b"node_hash_cache", &hashes_data) {
        return Err(BlockchainError::Storage(format!(
            "Failed to save node hash cache: {}",
            e
        )));
    }
