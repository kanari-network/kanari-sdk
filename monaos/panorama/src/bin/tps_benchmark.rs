        ...
        for i in batch_start..batch_end {
            ...
            let actual_complexity = match tx_type {
                ...
                TransactionType::Mixed => {
                    if rng.gen_bool(0.5) { complexity_factor } else { 0 }
                },
...
fn run_single_threaded(
    ...
    info!("Running single-threaded TPS benchmark with {} transactions", transaction_count);
    let addresses = setup_test_environment(account_count);
    
    // Generate transactions
    let gen_start = Instant::now();
    info!("Generating {} transactions", transaction_count);
    let transactions = generate_batch_transactions(&addresses, transaction_count, tx_type, complexity_factor);
    ...
    println!("Transaction count: {} (successful: {})", transactions.len(), success_count);
    println!("Transaction type: {:?} (complexity: {})", tx_type, complexity_factor);
    println!("Account count: {}", account_count);
    if let Some(conditions) = &network_conditions {
        println!("Network conditions: {}ms latency, {:.1}% failure rate", 
                conditions.latency_ms, conditions.failure_rate * 100.0);
    }
...
fn run_multi_threaded(
    ...
    info!("Running multi-threaded TPS benchmark with {} transactions on {} threads", 
    ...
    println!("Transaction count: {} (successful: {})", transactions_arc.len(), success_count);
...
fn run_generation_only(
    ...
    info!("Measuring transaction generation performance for {} transactions", transaction_count);
    ...
    println!("Transaction count: {} (generated: {})", transaction_count, transactions.len());
    println!("Transaction type: {:?} (complexity: {})", tx_type, complexity_factor);
    println!("Account count: {}", account_count);
    println!("Generation time: {:?}", gen_time);
    println!("Rate: {:.2} transactions per second", transactions.len() as f64 / gen_time.as_secs_f64());
    println!("=========================================");
}
...
fn run_network_simulation(
    ...
    info!("Running network simulation benchmark with {}ms latency and {:.1}% failure rate", 
          network_conditions.latency_ms, network_conditions.failure_rate * 100.0);
...
async fn main() {
    ...
    if success {
        println!("\n✅ TARGET ACHIEVED: The benchmark met or exceeded the target of {:.0} TPS!", args.target_tps);
    } else {
        println!("\n⚠️ TARGET NOT REACHED: The benchmark did not reach the target of {:.0} TPS.", args.target_tps);
    }
}
