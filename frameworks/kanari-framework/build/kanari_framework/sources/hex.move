module kanari_framework::hex {
    ...
    public fun encode(bytes: vector<u8>): vector<u8> {
        ...
        while (i < l) {
            ...
            let hex = vector::borrow(&HEX, (byte as u64));
            vector::append(&mut r, *hex);
            i = i + 1;
        };
        r
    }
    ...
    public fun decode(hex: vector<u8>): vector<u8> {
        let i = 0;
        let r = vector::empty<u8>();
        let l = vector::length(&hex);

        assert!(l % 2 == 0, EInvalidHexLength);
        while (i < l) {
            let high = decode_byte(*vector::borrow(&hex, i));
            let low = decode_byte(*vector::borrow(&hex, i + 1));
            let decimal = high * 16 + low;
