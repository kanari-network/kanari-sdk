module kanari_framework::object {
    use std::bcs;
    use kanari_framework::address;
    use kanari_framework::tx_context::{Self, TxContext};

    friend kanari_framework::clock;
    friend kanari_framework::coin;
    friend kanari_framework::dynamic_field;
    friend kanari_framework::dynamic_object_field;
    friend kanari_framework::transfer;
    friend kanari_framework::authenticator_state;
    ...
    const KARI_CLOCK_OBJECT_ID: address = @0x6;

    /// The hardcoded ID for the singleton AuthenticatorState Object.
    const KARI_AUTHENTICATOR_STATE_ID: address = @0x7;

    /// The hardcoded ID for the singleton Random Object.
    const KARI_RANDOM_ID: address = @0x8;

    /// The hardcoded ID for the singleton DenyList.
    const KARI_DENY_LIST_OBJECT_ID: address = @0x403;
    ...
    public(friend) fun clock(): UID {
        UID {
            id: ID { bytes: KARI_CLOCK_OBJECT_ID }
        }
    }
    ...
    public(friend) fun authenticator_state(): UID {
        UID {
            id: ID { bytes: KARI_AUTHENTICATOR_STATE_ID }
        }
    }
