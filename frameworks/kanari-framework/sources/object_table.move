...
/// Similar to `kanari_framework::table`, an `ObjectTable<K, V>` is a map-like collection. But unlike
/// `kanari_framework::table`, the values bound to these dynamic fields _must_ be objects themselves. This allows
/// for the objects to still exist within in storage, which may be important for external tools.
/// The difference is otherwise not observable from within Move.
module kanari_framework::object_table {
    use std::option::Option;
    use kanari_framework::object::{Self, ID, UID};
    use kanari_framework::dynamic_object_field as ofield;
    ...
    /// Aborts with `kanari_framework::dynamic_field::EFieldAlreadyExists` if the table already has an entry with
    /// that key `k: K`.
    ...
    /// Aborts with `kanari_framework::dynamic_field::EFieldDoesNotExist` if the table does not have an entry with
    /// that key `k: K`.
