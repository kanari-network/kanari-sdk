...
module kanari_framework::package {
    use kanari_framework::object::{Self, ID, UID};
    use kanari_framework::tx_context::{TxContext, sender};
    use std::ascii::String;
    use std::type_name;
    use kanari_framework::types;
    ...
    /// iskanari_frameworkng a ticket, so the ticket is a "Hot Potato" to preserve forward
    /// progress.
    ...
    public fun claim_and_keep<OTW: drop>(otw: OTW, ctx: &mut TxContext) {
        kanari_framework::transfer::public_transfer(claim(otw, ctx), sender(ctx))
    }
    ...
    public fun test_upgrade(ticket: UpgradeTicket): UpgradeReceipt {
        ...
        let package = object::id_from_bytes(kanari_framework::hash::blake2b256(&data));
