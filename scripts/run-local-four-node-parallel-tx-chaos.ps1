...
if ($FundSenders -and $CountPerSender -gt 1 -and $FaucetCoinsPerSender -lt 4) {
    Write-Warning "FaucetCoinsPerSender=$FaucetCoinsPerSender is intentionally low. Native transfers need distinct transfer/gas coin objects; low coin fanout serializes each lane and can look like a stuck node under load."
}
...
try {
    ...
    for ($i = 0; $i -lt $Senders.Count; $i++) {
        ...
        $txProcesses += Start-Process `
            ...
            -WindowStyle Hidden `
            -PassThru
        Write-Host "LANE $lane sender=$sender to=$laneRecipient rpc=$rpc pid=$($txProcesses[-1].Id)"
    }
