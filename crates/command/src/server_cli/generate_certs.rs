...
pub fn generate_ssl_certificates() -> Result<(), String> {
    println!("{}", "Generating SSL certificates for secure connections...".bright_green().bold());
    ...
    if cert_path.exists() && key_path.exists() {
        ...
        println!("  - Certificate: {}", cert_path.display().to_string().bright_white());
    ...
    println!("Generating certificate for node: {}", hostname.bright_white());
    ...
    let status = Command::new("openssl")
        .args(&[
            ...
            "-addext", &format!("subjectAltName=DNS:{}", hostname)
        ])
        .status();
    ...
    match status {
        Ok(exit_status) => {
            if exit_status.success() {
                println!("\n{}", "Successfully generated SSL certificates:".bright_green().bold());
                println!("  - Certificate: {}", cert_path.display().to_string().bright_white());
                println!("  - Key: {}", key_path.display().to_string().bright_white());
                println!("\nCertificates will be valid for 365 days.");
                Ok(())
            } else {
                Err(format!("Failed to generate certificates (exit code: {})", exit_status))
            }
        },
        Err(e) => {
            Err(format!("Failed to execute openssl command: {}\nMake sure OpenSSL is installed on your system.", e))
        }
    }
}
