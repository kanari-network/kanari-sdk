...
module kanari_system::rs256 {
    ...
    fun check_conditions_verify(
        ...
        assert!(
            vector::length(n) >= RSASSA_PKCS1_V1_5_MINIMUM_MODULUS_LENGTH / 8,
            ErrorInvalidPubKey
        );
        ...
        assert!(
            vector::length(n) <= (RSASSA_PKCS1_V1_5_MINIMUM_MODULUS_LENGTH / 8) * 2,
            ErrorInvalidPubKey
        );

        // 4. Ensure exponent (e) length meets minimum requirements (at least 1 byte)
        assert!(
            vector::length(e) >= RSASSA_PKCS1_V1_5_MINIMUM_EXPONENT_LENGTH,
            ErrorInvalidPubKey
        );

        // 5. Ensure exponent (e) length does not exceed maximum (512 bytes)
        assert!(
            vector::length(e) <= RSASSA_PKCS1_V1_5_MAXIMUM_EXPONENT_LENGTH,
            ErrorInvalidPubKey
        );
        ...
        if (e_len > 1) {
            ...
            assert!(
                leading_zeros < (e_len * 3 / 4), ErrorInvalidPubKey
            );
    ...
    fun check_conditions_verify_prehash(
        signature: &vector<u8>,
        n: &vector<u8>,
        e: &vector<u8>,
        msg: &vector<u8>,
