...
module kanari_system::dex_pool_tests {
    ...
    fun test_swap_with_fees() {
        ...
        assert!(
            amount_out_with_fee >= 89 && amount_out_with_fee <= 90,
            EInsufficientLiquidity
        ); // With fee case (may be same due to rounding)
    }
    ...
    fun test_minimum_liquidity_requirement() {
        ...
        assert!(
            initial_a < min_liquidity || initial_b < min_liquidity,
            EInsufficientLiquidity
        );
    }
    ...
    fun process_trades_recursive(
        ...
        if (index >= len) {
            ...
            if (new_k < k_initial) { false }
            else {
                // Continue with next trade
                process_trades_recursive(
                    new_reserve_a,
                    new_reserve_b,
                    k_initial,
                    trades,
    ...
    fun test_overflow_protection() {
        ...
        let amount_out = numerator / denominator;
        assert!(
            amount_out > 0 && amount_out < reserve_b, EInsufficientLiquidity
        );
    }
