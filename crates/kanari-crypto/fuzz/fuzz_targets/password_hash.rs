...
use kanari_crypto::{
    is_password_strong, hash_data, hash_data_with_algorithm,
    HashAlgorithm, secure_clear,
};
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(password) = std::str::from_utf8(data) {
        ...
        if password.len() < 16 {
            ...
            assert!(!is_strong, "Passwords with control characters should be weak");
        } else {
    ...
    assert_eq!(hash1, hash2, "hash_data and hash_data_with_algorithm should match");
    ...
    assert_eq!(hash5.len(), 32, "SHAKE256 default should produce 32-byte hash");

    // Test deterministic hashing
    let hash_again = hash_data(data);
    assert_eq!(hash1, hash_again, "Hashing same data should produce same result");

    // Test that different algorithms produce different hashes (for non-empty input)
    if !data.is_empty() {
        assert_ne!(hash1, hash3, "Different hash algorithms should produce different results");
    }
    ...
    if !data.is_empty() {
        ...
        assert!(test_vec.iter().all(|&b| b == 0), "All bytes should be zeroed");
        assert_eq!(test_vec.len(), original_len, "Length should remain unchanged");
    }
});
