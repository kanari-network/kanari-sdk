...
use kanari_crypto::{
    ...
    CurveType, extract_raw_key, format_private_key,
};
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(keypair) = generate_keypair(curve_type) {
        ...
        if let Some((parsed_curve, parsed_addr)) = kanari_crypto::keys::KeyPair::parse_tagged_address(&tagged) {
            ...
            assert!(!parsed_addr.is_empty(), "Parsed address should not be empty");
        } else {
    ...
    if matches!(curve_type, CurveType::K256 | CurveType::P256 | CurveType::Ed25519) {
        ...
        if let Ok(mnemonic_str) = std::str::from_utf8(key_data) {
            ...
            if mnemonic_str.split_whitespace().count() == 12 || mnemonic_str.split_whitespace().count() == 24 {
                let result = keypair_from_mnemonic(mnemonic_str, curve_type);
                
                // If mnemonic is valid, should succeed
                if result.is_ok() {
                    let kp = result.unwrap();
                    assert!(kp.address.starts_with("0x"), "Mnemonic-derived address should be valid");
                }
    ...
    if let Ok(pk_str) = std::str::from_utf8(key_data) {
        ...
        if pk_data.chars().all(|c| c.is_ascii_hexdigit() || c == '_') && !pk_str.is_empty() {
            let result = keypair_from_private_key(pk_str, curve_type);
            
            // May fail for invalid keys - that's acceptable
            if result.is_ok() {
                let kp = result.unwrap();
                assert!(kp.address.starts_with("0x"), "Imported key address should be valid");
            }
        }
    }
});
