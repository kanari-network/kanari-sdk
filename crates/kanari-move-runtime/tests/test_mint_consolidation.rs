...
fn test_self_transfer_should_not_duplicate_coins() -> Result<()> {
    ...
    assert_eq!(balance_after_mint2, 200, "Second mint should result in total 200");
    ...
    cs_transfer.created_objects.push(("coin_1_v2".to_string(), coin_1_remaining));
    ...
    cs_transfer.created_objects.push(("coin_1_split".to_string(), coin_1_split));
...
fn test_mint_then_self_transfer_real_scenario() -> Result<()> {
    ...
    cs_mint1.created_objects.push(("coin_1".to_string(), CreatedObject {
        owner: alice,
        uid: None,
        type_: format!("0x2::coin::Coin<{}>", token_type),
        data: coin1_data,
    ...
    state.apply_changeset(&cs_mint1)?;
    let bal1 = state.get_account(&alice).expect("Alice exists").get_token_balance(token_type);
    ...
    cs_mint2.created_objects.push(("coin_2".to_string(), CreatedObject {
        owner: alice,
        uid: None,
        type_: format!("0x2::coin::Coin<{}>", token_type),
        data: coin2_data,
    ...
    state.apply_changeset(&cs_mint2)?;
    let bal2 = state.get_account(&alice).expect("Alice exists").get_token_balance(token_type);
    ...
    let bal3 = state.get_account(&alice).expect("Alice exists").get_token_balance(token_type);
