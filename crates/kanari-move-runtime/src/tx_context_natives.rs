    ...
    let sender = match tx_context_ref.value_as::<StructRef>() {
        Ok(struct_ref) => {
            ...
            match struct_ref.borrow_field(0) {
                Ok(sender_value) => {
                    match sender_value.value_as::<AccountAddress>() {
                        Ok(addr) => addr,
...
pub fn native_epoch(
    ...
    let epoch = match tx_context_ref.value_as::<StructRef>() {
        Ok(struct_ref) => {
            match struct_ref.borrow_field(2) {
...
pub fn native_epoch_timestamp_ms(
    ...
    let timestamp = match tx_context_ref.value_as::<StructRef>() {
        Ok(struct_ref) => {
            match struct_ref.borrow_field(3) {
...
pub fn tx_context_natives(
    addr: AccountAddress,
) -> Vec<(AccountAddress, move_core_types::identifier::Identifier, move_core_types::identifier::Identifier, NativeFunction)> {
