...
use get_block::{
    get_account_details, get_all_blocks, get_gas_fee_info, get_transaction_by_id,
    get_transaction_status, search_transactions,
};
        ...
        .start_http(&bind_addr)
    {
        Ok(server) => {
            ...
            if network_config.localhost_only {
                ...
                println!(
                    "HTTP server running on http://127.0.0.1:{}",
                    network_config.port
                );
                println!(
                    "Note: The node will not connect to or accept connections from other nodes"
                );
            } else {
                println!(
                    "HTTP server running on http://{}:{}",
                    network_config.node_address, network_config.port
                );
                        ...
                        for env in envs {
                            if let Some(alias) = env.get("alias").and_then(|v| v.as_str()) {
                                if alias == active_env {
                                    ...
                                    env["rpc"] = serde_yaml::Value::String(format!(
                                        "http://127.0.0.1:{}",
                                        network_config.port
                                    ));
                                    break;
