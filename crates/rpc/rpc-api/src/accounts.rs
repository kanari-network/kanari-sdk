...
use mona_blockchain::{
    blockchain::{BALANCES, BLOCKCHAIN_DATA, load_blockchain_with_retry},
    chain_id::CHAIN_ID,
};
...
pub fn get_blockchain_status(_params: Params) -> JsonRpcResult<JsonValue> {
    ...
    let total_transactions = BLOCKCHAIN_DATA
        .iter()
        .into_iter()
...
pub fn list_accounts(_params: Params) -> JsonRpcResult<JsonValue> {
    ...
    for (address_str, balance) in balances.iter() {
        ...
        let tx_count = BLOCKCHAIN_DATA.iter().into_iter().fold(0, |acc, block| {
...
pub fn transfer_tokens(params: Params) -> JsonRpcResult<JsonValue> {
    ...
    let transfer_params: TransferParams = params
        .parse()
            ...
            match process_transfer(
                ...
                Ok(transaction) => {
                    ...
                    let signature_status = if transaction.signature.is_empty() {
                        ...
                        match panorama::transfer_tokens::verify_transaction::verify_transaction(
                            &transaction,
                        ) {
                            Ok(true) => "valid",
                            Ok(false) => "invalid",
