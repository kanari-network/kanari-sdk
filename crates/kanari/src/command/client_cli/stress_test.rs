...
const SEQUENCE_RETRY_DELAY_MS: u64 = 50;
...
fn is_sequence_race_error(error: &anyhow::Error) -> bool {
    let message = format!("{:#}", error);
    message.contains("Sequence number too high") || message.contains("Sequence number too low")
}
        ...
        for i in 0..self.count {
            ...
            let result = loop {
                ...
                match attempt_result {
                    Err(error)
                        if is_sequence_race_error(&error) && retry_count < MAX_SEQUENCE_RETRIES =>
                    {
                        retry_count += 1;
                        eprintln!(
                            "  [{}/{}] sequence changed while submitting; rebuilding tx (retry {}/{})",
                            i + 1,
                            self.count,
                            retry_count,
                            MAX_SEQUENCE_RETRIES
                        );
                        sleep(Duration::from_millis(SEQUENCE_RETRY_DELAY_MS)).await;
                    }
                    other => break other,
                }
            };
