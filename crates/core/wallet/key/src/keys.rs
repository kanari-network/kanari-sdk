...
use bip39::{rand::rngs::OsRng, Language, Mnemonic, rand};
    ...
    match curve_type {
        CurveType::K256 => {
            let secret_key = K256SecretKey::from_slice(bytes)
                .map_err(|_e| KeyError::InvalidPrivateKey)?;
...
pub fn generate_mnemonic(word_count: usize) -> Result<String, KeyError> {
    let mnemonic_result = match word_count {
        ...
        _ => return Err(KeyError::GenerationFailed(
            format!("Unsupported word count: {}", word_count)
        )),
    };
