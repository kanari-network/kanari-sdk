...
// Add necessary signing and verification functions
pub fn sign_message(message: &str, private_key: &str) -> Result<String, String> {
    ...
    let private_key_bytes = match hex::decode(private_key_clean) {
        Ok(bytes) => bytes,
        Err(e) => return Err(format!("Invalid private key format: {}", e))
    };
    
    // Create a secp256k1 secret key
    let secret_key = match secp256k1::SecretKey::from_slice(&private_key_bytes) {
        Ok(key) => key,
        Err(e) => return Err(format!("Invalid private key: {}", e))
    };
    
    // Hash the message (using keccak256 for Ethereum compatibility)
    ...
    let msg = match Message::from_slice(&hash) {
        Ok(msg) => msg,
        Err(e) => return Err(format!("Message creation error: {}", e))
    };
...
pub fn verify_signature(message: &str, signature_hex: &str, address: &str) -> Result<bool, String> {
    ...
    // Try all possible recovery IDs (0, 1, 2, 3)
    for recovery_id in 0..4 {
        if let Ok(recovery_id) = secp256k1::ecdsa::RecoveryId::from_i32(recovery_id) {
            // Create a recoverable signature from the signature and recovery ID
            let recoverable_sig = match secp256k1::ecdsa::RecoverableSignature::from_compact(
                &signature.serialize_compact(), 
                recovery_id
            ) {
                Ok(sig) => sig,
                Err(_) => continue, // Try next recovery ID if this fails
            };
            
            // Use recover_ecdsa with the correct signature type
            if let Ok(pubkey) = secp.recover_ecdsa(&msg, &recoverable_sig) {
                // Calculate Ethereum-style address from public key
                let mut hasher = Keccak256::new();
