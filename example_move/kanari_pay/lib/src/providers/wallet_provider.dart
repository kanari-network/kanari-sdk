...
class WalletState extends ChangeNotifier {
  ...
  Future<void> setEnvironment(KanariEnvironment env) async {
    ...
    if (_wallet != null) {
      _setLoading(true); // 👉 1. Show loading state when switching networks

      // 👉 2. Clear old balance immediately to prevent Dev network balance from showing in Local
      _balance = 0;
      _tokenBalances = [];
      notifyListeners();

      await refreshBalance(); // Fetch data for new network

      _setLoading(false); // 👉 3. Hide loading state
    } else {
      notifyListeners();
    }
  }
  ...
  Future<void> addWallet(Map<String, dynamic> walletData, [String? pin]) async {
    ...
    // Save PIN to storage for the very first wallet
  ...
  Future<void> refreshBalance() async {
    if (_client != null && _wallet != null) {
      try {
        ...
        // 👉 4. If fetching fails (e.g., server is down), clear values to 0
        _balance = 0;
        _tokenBalances = [];
        _error = "Refresh balance failed: $e";
        debugPrint(
          _error,
        ); // Check Console to see if error is due to connection failure or not
        notifyListeners();
      }
    }
  }
