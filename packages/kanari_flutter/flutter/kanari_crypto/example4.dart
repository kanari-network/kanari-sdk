...
Future<void> main() async {
  ...
  try {
    ...
    final desired = 'Dilithium3';
    if (!curves.any((c) => c.name == desired)) {
      final available = curves.map((c) => c.name).join(', ');
      print('Dilithium3 not available. Available curves: $available');
      return;
    }
    ...
    final message = 'kanari pq test dilithium3'.codeUnits;
    ...
    stderr.writeln('Example4 failed: $e');
    stderr.writeln(st);
  } finally {
    RustLib.dispose();
  }
